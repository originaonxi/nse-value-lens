...
const {validSnapshot: validBrief} = require('../public/market-brief');
const cache = new Map(), inflight = new Map();
async function loadSwing(name) {
  if (!['swing_desk','swing_evidence','expanded_research','cross_asset_research','futures_research','research_data_audit','indicator_research','hhhl_scan','hhhl_refresh_status','vcp_scan','vcp_refresh_status','market_brief','market_brief_refresh_status'].includes(name)) throw new Error('Unknown swing dataset');
  ...
  const request = (async () => {
    ...
    try {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(payload.as_of || '') ||
          (name === 'market_brief' ? !validBrief(payload) : name === 'vcp_scan' ? !validVcp(payload) : name === 'hhhl_scan' ? !(payload.universe_count === 200 && Array.isArray(payload.rows) && payload.rows.length === 200 && new Set(payload.rows.map(r=>r.symbol)).size === 200 && payload.market) : name.endsWith('_refresh_status') ? !(['fresh','partial','waiting','failed'].includes(payload.state) && payload.run_id && payload.attempted_at) : name === 'swing_desk' ? !Array.isArray(payload.candidates) : name === 'research_data_audit' ? !Number.isFinite(payload.opening_bars) : !payload.results))
